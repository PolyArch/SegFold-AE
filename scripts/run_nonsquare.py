#!/usr/bin/env python3
"""Non-Square Performance: Run SegFold on 6 rectangular SuiteSparse matrices.

Reproduces the paper's non-square speedup comparison figure.
Spada baselines are pre-computed in data/baselines/nonsquare_baselines.csv.

Uses csegfold --mtx-file for CSR-only simulation (no dense conversion).

Usage:
    python3 scripts/run_nonsquare.py output/my_run
    python3 scripts/run_nonsquare.py output/my_run --jobs 4
"""
...
def main():
    parser = argparse.ArgumentParser(description="Non-square performance experiment")
    ...
    out_dir = args.output_dir / "nonsquare"
    out_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 50)
    print("Non-Square Performance Experiment")
